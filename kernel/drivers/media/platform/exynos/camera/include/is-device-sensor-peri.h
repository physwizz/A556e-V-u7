...
struct is_flash_data {
	...
	struct work_struct		flash_set_pre_config_work;
...
struct is_ois {
	...
	int				ois_power_mode;
	struct work_struct			set_init_work;
	struct work_struct			set_deinit_work;
	int				af_pos_wide;
...
struct is_mcu {
	struct v4l2_subdev			*subdev; /* connected module subdevice */
	struct i2c_client			*client;
#if defined(CONFIG_CAMERA_USE_AOIS)
	struct devicemgr			*dev;
#endif
	u32						id;
	u32						device; /* connected sensor device */
	u32						ver;
