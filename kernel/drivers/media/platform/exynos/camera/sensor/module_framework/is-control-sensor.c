					...
					struct ae_param adj_dgain)
{
	...
	if (expo.val == 0 || adj_again.val == 0 || adj_dgain.val == 0) {
		dbg_sensor(1, "[%s] skip set_totalgain (%d, %d, %d)\n",
				__func__, expo.val, adj_again.val, adj_dgain.val);
		return ret;
	}
...
void is_sensor_ctl_frame_evt(struct is_device_sensor *device)
{
	...
	applied_frame_number = ZERO_IF_NEG(vsync_count - sensor_peri->sensor_interface.diff_bet_sen_isp);
