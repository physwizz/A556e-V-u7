...
static void decon_fingerprint_mask(struct exynos_drm_crtc *crtc, u32 after)
{
	struct drm_connector *conn =
		crtc_get_conn(crtc->base.state, DRM_MODE_CONNECTOR_DSI);
	struct decon_device *decon;
	...
	if (conn) {
		...
		if (funcs && funcs->set_fingermask_layer)
			funcs->set_fingermask_layer(exynos_conn, after);
	}
			       ...
			       struct drm_atomic_state *old_state)
{
	...
	decon_reg_set_rcd_enable(decon->id, decon->config.rcd_en);
...
#if IS_ENABLED(CONFIG_USDM_PANEL_MASK_LAYER)
	exynos_crtc->ops->set_fingerprint_mask(exynos_crtc, 0);
#endif
...
decon_enable(struct exynos_drm_crtc *crtc, struct drm_atomic_state *old_state)
{
	...
	if (__is_recovery_supported(decon) && __is_recovery_uevent_clear(decon)) {
		exynos_recovery_set_state(decon, RECOVERY_IDLE);
		exynos_conn_state->bypass_panel = false;
	}
#endif
...
static int decon_parse_dpps(struct decon_device *decon, struct device_node *np)
{
	...
	if (dpp_np) {
		...
		if (dpp_pdev) {
			...
			decon_info(decon, "found rcd: dpp%d\n", decon->rcd->id);
		}
		of_node_put(dpp_np);
	} else {
		decon_info(decon, "can't find rcd node\n");
	}
